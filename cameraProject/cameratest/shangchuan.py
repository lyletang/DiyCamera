...
# UI callbacks -------------------------------------------------------------
def quitMessage():
	tmp = tkMessageBox.askyesno(title='EXIT',message='Are you exit?')
	if temp == True:
		quitCallback()
	else:
		pass
...
def imageCallback(n):       #照相消息处理
  ...
  if n is 2:
	...
	threads = []
	t1 = threading.Thread(target=renlianjiance, args=())
	threads.append(t1)
...
buttons = [
  ...
  [Button(( 10,214, 52, 52), bg='photo_small', cb=viewCallback, value=0),
   ...
   Button((738,316, 52, 52), bg='quite', cb=quitMessage),
...
#the picture renlian jiance
  ...
  [Button((738, 10, 52, 52), bg='up', cb=videoCallback, value=1),
   ...
   #the button of upload videos
   Button(( 10,367, 52, 52), bg='interfere',cb=videoCallback,value=15),
...
def renlianjiance():
	...
	if os.path.exists(pathData[storeMode][0] + '/IMG_' + '%04d' % image_loadIdx + '.jpg'):
		imagePath = pathData[storeMode][0] + '/IMG_' + '%04d' % image_loadIdx + '.jpg'
	cmdStr = 'python jiance01.py' + ' ' + imagePath
	os.system(cmdStr)
'''
def jiance():
	threads = []
...
def upload():
	cmdStr = 'scp -r /home/pi/cameraProject/Videos/ tangjiahui@192.168.1.119:/home/tangjiahui/upload/'
	os.system(cmdStr)
# Initialization -----------------------------------------------------------
