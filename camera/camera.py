...
def viewCallback(n): # Viewfinder buttons
  global scan_flag, scaled, screenMode, screenModePrior,pin_flag
...
def videoRange(path):
  ...
  try:  
    for file in os.listdir(path):
      if fnmatch.fnmatch(file, 'VID_[0-9][0-9][0-9][0-9].mjpeg'):
        i = int(file[4:8])
        if(i < min): min = i
        if(i > max): max = i
  finally:
...
def takeVideo():
  ...
  if video_flag:
    camera.stop_preview()
    while True:
      filename = pathData[storeMode][1] + '/VID_' + '%04d' % video_saveIdx + '.mjpeg'
...
def showNextVideo(direction):
  ...
  while True:
    ...
    if os.path.exists(pathData[storeMode][1] + '/VID_' + '%04d' % n + '.mjpeg'):
        showVideo(n)
        break
...
def showVideo(n):
  ...
  video_loadIdx = n
  filenamestr = pathData[storeMode][1] + '/VID_' + '%04d' % n + '.mjpeg'
