...
four_scaled     = [None, None, None, None]
screenWidth	= 800
screenHeight	= 480
video_flag	= True
...
def takeVideo():
  ...
  if video_flag:
    ...
    video_flag = False
    #t3 = thread.start_new_thread(timershowTime,(1,1)) 
  else:
    camera.stop_recording()
    video_flag = True
def timershowTime():
  while (dt.datetime.now() - start_time) < 20:
    camera.annotate_text = dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
  ...
  while mem > 0:
     ...
     while True:
       ...
       if os.path.exists(pathData[storeMode][0] + '/IMG_' + '%04d' % n + '.JPG'):
         ...
         four_scaled[3 - mem] = pygame.transform.scale(img, (320, 240))
         print n
    	 break
...
while True:
  ...
  if cflag:
    ...
    if screenMode == 3:
      if four_scaled[0]:
        screen.blit(four_scaled[0],
          ((800 - four_scaled[0].get_width() ) / 2 - 160,
           (480 - four_scaled[0].get_height()) / 2 - 120))
      if four_scaled[1]:
        screen.blit(four_scaled[1],
          ((800 - four_scaled[1].get_width() ) / 2 + 160,
           (480 - four_scaled[1].get_height()) / 2 - 120))
      if four_scaled[2]:
        screen.blit(four_scaled[2],
          ((800 - four_scaled[2].get_width() ) / 2 - 160,
           (480 - four_scaled[2].get_height()) / 2 + 120))
      if four_scaled[3]:
        screen.blit(four_scaled[3],
          ((800 - four_scaled[3].get_width() ) / 2 + 160,
           (480 - four_scaled[3].get_height()) / 2 + 120))
